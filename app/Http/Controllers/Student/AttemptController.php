...
public function submit(Attempt $attempt)
{
    ...
    foreach ($attempt->test->questions as $question) {
        ...
        // Если вопрос не имеет ответа — пропускаем (не начисляем баллы)
        if ($userAnswers->isEmpty()) {
            continue;
        }

        $isCorrect = false;
        $questionPoints = $question->points ?? 1; // если поле points не задано, по умолчанию 1
        ...
        if ($question->type == 'single_choice') {
            ...
            // Если все выбранные варианты правильные и выбраны все правильные (строгое совпадение)
...
// \Log::info('Question ID: ' . $question->id . ', Points: ' . $questionPoints);
// \Log::info('User answers: ' . json_encode($userAnswers->toArray()));
...
public function saveAnswer(Request $request, Attempt $attempt)
{
    ...
    if ($question->type == 'single_choice' && isset($data['choice_id'])) {
        // Удаляем старый ответ и создаём новый
